...
export default function RecipesScreen() {
  ...
  // Filter recipes by category or search term
  const displayedRecipes = recipes.filter(currentRecipe => {
    if (category) {
      return currentRecipe.category === category;
    }
    if (search) {
      return currentRecipe.title.toLowerCase().includes(search.toLowerCase());
    }
    return true;
  });
